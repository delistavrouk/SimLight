...
import platform
from pyvis.network import Network   # https://pyvis.readthedocs.io/en/latest/install.html
...
def generateDistributedTrafficRequestValuesThatFollowThePoissonProcess(x, num):
    ...
    #print("Poisson traffic request distributes values")
    #print(data)
...
'''
def find_shortest_path_using_bfs(graph, src, dst): OLD not working due to false data structure
    ...
    #SOP
    if (GlobalPrintOutEnabled==True) :
        print("<li>[Find shortest path] Graph:",graph,"<li>Source:",src,"<li>Destination:",dst)
    #EOP

    # if the start and destination nodes are the same return the source node as the path
    if src == dst:
        print("<li>[Find shortest path] src==dst",src,"=",dst)
        return [src]

    # dictionary to be aware of the visited nodes
    visited = {}
    for node in graph:
        visited[node] = False
    print("<li>[Find shortest path] Visited",visited)

    # define a queue for breadth first search and initialize it with the source node
    queue = deque()
    
    # queue items are tuples (node, path)
    queue.append((src, [src]))  

    print("<li>[Find shortest path] Queue",queue)

    # check that the source node is visited
    visited[src] = True
    ...
    while queue:
        ...
        print("<li>[Find shortest path] current node",current_node)
        print("<li>[Find shortest path] path",path)

        # Explore neighbors of the current node
        # https://realpython.com/python-keyerror/
        # https://stackoverflow.com/questions/15653966/ignore-keyerror-and-continue-program
        # https://www.digitalocean.com/community/tutorials/python-keyerror-exception-handling-examples

        try: # exception handling: since I use only the virtual links of the virtual topology that have adequate capacity, hence VT2 is fragmented and some nodes that exist in VT might not exist in VT2
            for neighbor in graph[current_node]:
                print("<li>[Find shortest path] Graph current node",graph[current_node])
                print("<li>[Find shortest path] neighbor",neighbor)

                if not visited[neighbor]:

                    print("<li>[Find shortest path] visited neighbor",visited[neighbor])

                    if neighbor == dst:
                        # this is the shortest path!
                        print("<li>[Find shortest path] path + neighbor",path + [neighbor])
                    ...
                    queue.append((neighbor, path + [neighbor]))
                    print("<li>[Find shortest path] queue appended",queue)

        except KeyError:   # if a node is not found in the graph or the nodes marked as visited, then ignore the error and continue
            continue

    print("<li>[Find shortest path] no path found []")

    # in case all the queue is traversed and no path is found, return an empty list
    return []
'''
